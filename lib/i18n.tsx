...
const DICT: Record<string, { en: string; zh: string; ja: string }> = {
  ...
  "editor.actorNotHere": { en: "This step cannot be performed at random — a pick belongs to whoever made it. Choose a player.", zh: "這個步驟不能交給隨機 —— 選出來的東西要有歸屬。請改指定給選手。", ja: "このステップはランダムにできません。選んだものには持ち主が必要です。プレイヤーを指定してください。" },
  ...
  "validate.offerOverHand": { en: "Game {game} asks a player to offer {want} civs, but they only have {have} to give — the step can never complete and the draft stops there.", zh: "第 {game} 場要求選手拿出 {want} 個文明出戰，但他手上只有 {have} 個 —— 這個步驟永遠無法完成，抽選會停在那裡。", ja: "ゲーム {game} は出場文明を {want} 個要求しますが、手持ちは {have} 個しかありません。このステップは完了できず、ドラフトがそこで止まります。" },
  ...
  "validate.drawCannotPick": { en: "Step {step} is assigned to “Random”, but a random draft has no owner — the civ would land in nobody's hand, so nobody is put on the clock and the draft cannot move. Use P1, P2, winner or loser.", zh: "第 {step} 個步驟指定給「隨機」，但隨機抽出來的文明不屬於任何人 —— 不會有人被輪到，抽選會卡住。請改用 P1、P2、勝方或敗方。", ja: "ステップ {step} は「ランダム」に割り当てられていますが、ランダムに引いた文明は誰のものにもならないため、誰の番にもならずドラフトが進みません。P1・P2・勝者・敗者を使ってください。" },
  "validate.drawNoMapsLeft": { en: "Step {step} draws the map at random from the maps neither player claimed, but by then the bans and picks have used the whole pool — there is nothing left to draw. Free up a map, or let a player choose this one instead of the draw.", zh: "第 {step} 個步驟要從「兩邊都沒有拿走的地圖」裡隨機抽，但排到這一步時禁用與選用已經把整個地圖池用光 —— 沒有東西可以抽。請留幾張地圖給隨機，或把這一步改成由選手指定。", ja: "ステップ {step} はどちらのプレイヤーも取らなかったマップからランダムに選びますが、その時点で禁止と選択がマッププールを使い切っており、抽選できるマップが残っていません。マップを残すか、このステップをプレイヤーの選択に変更してください。" },
